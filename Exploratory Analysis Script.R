...
# Ponemos los precios en base 1 para el primer dato
b1_WBK <- df_cotizaciones$WBK.Adjusted/df_cotizaciones$WBK.Adjusted[1]
b1_REG <- df_cotizaciones$REG.Adjusted/df_cotizaciones$REG.Adjusted[1]
b1_ISP <- df_cotizaciones$ISP.MI.Adjusted/df_cotizaciones$ISP.MI.Adjusted[1]
b1_BAYN <- df_cotizaciones$BAYN.DE.Adjusted/df_cotizaciones$BAYN.DE.Adjusted[1]
b1_ALV <- df_cotizaciones$ALV.DE.Adjusted/df_cotizaciones$ALV.DE.Adjusted[1]
b1_MSCI <- df_cotizaciones$MSCI.Adjusted/df_cotizaciones$MSCI.Adjusted[1]

plot(b1_WBK,type="l", col= "orange", ylim=c(0.6,3.5))
lines(b1_REG, col = "blue")
lines(b1_ISP, col="red")
lines(b1_BAYN, col="brown")
lines(b1_ALV, col="purple")
lines(b1_MSCI)
...
cartera_acumulada <- 0.2*df_cotizaciones$WBK.Adjusted +
  0.2*df_cotizaciones$REG.Adjusted +
  0.2*df_cotizaciones$ALV.DE.Adjusted +
  0.2*df_cotizaciones$BAYN.DE.Adjusted +
  0.2*df_cotizaciones$ISP.MI.Adjusted 

cartera_acumulada_100 <- cartera_acumulada / cartera_acumulada[1]
cartera_acumulada_100
benchmark_100 <- df_cotizaciones$MSCI.Adjusted / df_cotizaciones$MSCI.Adjusted[1]
benchmark_100

plot(benchmark_100, type= "l", ylim=c(0.75, 3.5), lwd= 2, col="blue", xlab="Día", ylab = "Rendimiento (base 1)")
lines(cartera_acumulada_100, lwd=2, col="red")
legend(10,3.5,legend=c("MSCI","Cartera seleccionada"), col=c("blue","red"),lty=1:2,cex=0.8)
